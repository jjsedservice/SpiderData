...
DEFAULT_INPUT = Path("/Users/lastjob/Downloads/风电场3.csv")
DEFAULT_OUTPUT = Path("/Users/lastjob/Downloads/风电场3_带经纬度.csv")
DEFAULT_CACHE = Path("/Users/lastjob/Documents/Work/SpiderData/tianditu_windfarm_cache.json")
DEFAULT_TIMEOUT = 20
...
def build_queries(row: Dict[str, str]) -> List[str]:
    ...
    plant_name = clean_text(row.get("电场名称"))
    if not plant_name:
        return []

    queries: List[str] = []
    full_query = f"{province}{plant_name}"
    if full_query:
        queries.append(full_query)

    trimmed_name = plant_name
    match = re.search(r"(电场|风电)", trimmed_name)
    if match:
        trimmed_name = trimmed_name[:match.start()].strip()
    trimmed_name = re.sub(r"（[^）]*）|\([^)]*\)", "", trimmed_name).strip()

    trimmed_query = f"{province}{trimmed_name}"
    if trimmed_name and trimmed_query not in queries:
...
def geocode_row(
    ...
    for query in queries:
        ...
        last_cached = cached
        result["查询地址"] = query
        result["定位级别"] = cached.get("level", "province_name")
        ...
        if cached.get("status") == "success":
            ...
            result["省"] = clean_text(cached.get("province")) or clean_text(result.get("省"))
            result["市"] = clean_text(cached.get("city"))
            result["县"] = clean_text(cached.get("county"))
            return result
...
def geocode_csv(
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as infile:
        ...
        output_fieldnames = fieldnames + [column for column in OUTPUT_COLUMNS if column not in fieldnames]
